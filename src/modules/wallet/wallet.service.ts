...
export class WalletService {
    constructor(private db:PrismaService,private loggerService:LoggerService){}
    ...
    async createWallet(userId,walletDto:WalletCreateDto){
        ...
        if(!wallet){
            this.loggerService.error("Could not create wallet","Wallet Service","",{
            userId:walletDto.userId
            })
            throw new BadRequestException(`Could not create wallet for user ${walletDto.userId}`)
        }
        return {
    ...
    async walletIncrement(userId:string,walletIncrement:WalletOperationDto){
        ...
        const wallet = await this.db.$transaction(async(tx)=>{
            ...
            if(!wallet){
                this.loggerService.error(`Wallet for the user id ${userId} could not be found`,"Wallet Service","",{
                    userId
                })
                throw new NotFoundException(`Wallet for the user id ${userId} could not be found`)
            }
    ...
    async walletDecrement(userId:string,walletDecrement:WalletOperationDto){
        ...
        const wallet = await this.db.$transaction(async(tx)=>{
            ...
            if(!wallet){
                this.loggerService.error(`Could not find wallet for user ${userId}`,"Wallet Service","",{
                    userId:userId
                })
